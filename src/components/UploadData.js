...
function UploadData(props) {
  ...
  return (
    <>
      ...
      <button type="submit" title={accessToken ? 'Upload steps to Google Fit' : 'Please sign in to upload'} className={accessToken && treadmillData.steps > 0 && !treadmillData.uploaded ? 'button' : 'disabledButton'} onClick={uploadData} disabled={!(accessToken && treadmillData.steps > 0 && !treadmillData.uploaded)}>
        <span id="uploadLoading" className="hidden" />
        <div id="uploadText" className="buttonDiv">
          <img src={gfit} alt="Google Fit Logo" />
          Upload to Google Fit
        </div>
      </button>
    </>
  );
